...
const statusDetails = {
    ...
    en_cours_de_traitement: { label: "Traité", color: "text-blue-600", bgColor: "bg-blue-100" },
...
const ApplicationPreviewA4 = ({ request, onClose }) => {
    ...
    const getDocumentUrl = (filePath: string) => {
        ...
        // Correction de l'URL pour les documents privés
        return `${import.meta.env.VITE_API_URL.replace('/api', '')}/private_uploads/permis_international/${request.reference}/${fileName}`;
    };
    
    const photoPath = request.documents?.find(doc => doc.type_document === 'photo_identite')?.chemin_fichier;
    const photoUrl = getDocumentUrl(photoPath);
    ...
    return (
        <div className="printable-area">
            <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 non-printable">
                <div className="bg-gray-50 rounded-lg shadow-2xl w-full max-w-5xl h-full max-h-[95vh] flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-6">
                        <div className="w-[210mm] min-h-[297mm] mx-auto bg-white p-8 shadow-lg border">
                            {/* Le contenu de la page A4 est déjà ici */}
                        </div>
                    </div>
                </div>
            </div>

            {/* This is the actual content that will be printed, positioned correctly by the CSS */}
            <div className="printable-content bg-white p-8">
                ...
                <footer className="text-center text-xs text-gray-500 border-t-2 border-gray-300 pt-4 mt-12">
                    ...
                    <div className="flex items-center justify-center gap-4 mt-2">
                        <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> BP: 738 Niamey - Niger</span>
                        <span className="flex items-center gap-1"><Phone className="w-3 h-3" /> Tél: +227 20 72 26 41</span>
                        <span className="flex items-center gap-1"><Mail className="w-3 h-3" /> contact@transport.ne</span>
                    </div>
                </footer>
            </div>
        </div>
